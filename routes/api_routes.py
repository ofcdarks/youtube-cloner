...
from routes._seed_data import _ROBOS_SEED_NICHES, _ROBOS_SEED_TITLES, _RELATOS_SEED_NICHES, _RELATOS_SEED_TITLES
...
Retorne APENAS o JSON.{lang_instruction}"""
        ...
        short_titles = [idea for idea in new_ideas if len(idea.get("title", "")) < 50]
        if short_titles:
            logger.info(f"Expanding {len(short_titles)} short titles (<70 chars)")
            short_list = "\n".join([f'- "{t["title"]}" ({len(t["title"])} chars)' for t in short_titles])
            expand_prompt = f"""Estos {len(short_titles)} titulos de YouTube son DEMASIADO CORTOS (menos de 50 caracteres).
Reescribe CADA UNO para que tenga ENTRE 50 y 80 caracteres, manteniendo el mismo tema y emocion.

TITULOS CORTOS:
{short_list}

REGLAS:
- MINIMO 50 caracteres, MAXIMO 80 caracteres por titulo
...
JSON: [{{"original": "titulo original", "expanded": "titulo expandido 50-80 chars"}}]
Solo JSON."""
            try:
                expand_resp = chat(expand_prompt, system="Expande titulos cortos de YouTube a 50-80 caracteres.", max_tokens=4000, temperature=0.7)
                expand_match = re.search(r'\[.*\]', expand_resp, re.DOTALL)
                if expand_match:
                    expansions = json.loads(expand_match.group())
                    expand_map = {e["original"]: e["expanded"] for e in expansions if len(e.get("expanded", "")) >= 50}
                    replaced = 0
