...
@router.get("/api/seed-rescue")
async def seed_rescue(request: Request):
    """One-time seed: RESCUE project."""
    from database import get_projects, create_project, save_niche, save_idea, save_file, log_activity, get_db
    existing = [p for p in get_projects() if "RESCUE" == p.get("name", "")]
    if existing:
        return JSONResponse({"ok": True, "msg": "RESCUE already exists", "id": existing[0]["id"]})
    with get_db() as conn:
    ...
    pid = create_project(name="RESCUE", channel_original="", niche_chosen="Animal Rescue Documentary", language="en")
    sop = """# RESCUE SOP — Animal Rescue Documentary
DNA: Dramatic animal rescue storytelling. AI-generated cinematic visuals. Emotional arc: danger->rescue->recovery->reunion.
FORMAT: 8-20 min. AI-generated photorealistic/3D animal scenes, dramatic narration, emotional music swells.
TITLE FORMULAS: "A [baby animal] Runs Away Seeking Help | What Happens Next...", "ANIMAL RESCUE: A [animal] [trapped/chained/abandoned] [dramatic situation]", "[Animal] Trapped On [dangerous place] Will Anyone Save It?"
HOOK: Immediate danger. Animal in peril. Ticking clock. "A baby tiger cub runs through the jungle alone. Its mother is nowhere to be found..."
STRUCTURE: Discovery of danger -> Failed first attempt -> Escalation -> Heroic rescue -> Recovery montage -> Emotional reunion/release
TONE: Dramatic, suspenseful, heartwarming. Heavy emotional manipulation. Narrator builds tension then releases with hope.
GOLDEN RULES: 1.Animal in danger within 10s 2.Name the animal for empathy 3.Ticking clock always 4.Setback before success 5.Villain clear (poacher/storm/human) 6.Rescuer is hero 7.Recovery montage 8.Reunion is emotional peak 9.Sound design described 10.Baby animals get more clicks 11.Never show graphic injury 12.Hope must win 13.End with release/freedom 14.Music swells at rescue 15.CTA after emotional peak
VISUAL: Photorealistic/3D cinematic animal scenes, jungle/savanna/ocean environments, dramatic lighting, rain/storm effects, golden hour for happy endings
RPM: $4-10. Audience: All ages, animal lovers, families, emotional content seekers."""
    save_file(pid, "analise", "SOP - RESCUE (NotebookLM)", f"sop_{pid}.md", sop)
    niches = [
        ("Big Cat Rescues", "Tigers, lions, leopards, cheetahs — cubs separated from mothers, trapped, poached, chained", "$5-10", "Alta", "#FF8C00", True),
        ("Ocean & Marine Rescues", "Whales, dolphins, sea turtles, sharks — stranded, tangled in nets, pollution, beached", "$4-8", "Media", "#006994", True),
        ("Baby Animal Rescues", "Orphaned cubs, abandoned puppies, lost baby elephants, injured fawns — maximum empathy", "$5-12", "Media", "#8B4513", False),
        ("Safari & Savanna Rescues", "Elephants, giraffes, rhinos, zebras — poachers, drought, wildfires, floods", "$4-8", "Baja", "#228B22", False),
        ("Extreme Weather Rescues", "Animals trapped in floods, wildfires, hurricanes, earthquakes, volcanic eruptions", "$5-10", "Baja", "#4169E1", False),
    ]
    for name, desc, rpm, comp, color, chosen in niches:
        save_niche(pid, name, desc, rpm, comp, color, chosen)
    titles = [
        ("A Baby Tiger Runs Away Seeking HELP | What the Rescue Team Found...", "Big Cat Rescues", "ALTA", 74000),
        ("ANIMAL RESCUE: A Tiger Mother CHAINED on Train Tracks | Will Her Cubs Survive?", "Big Cat Rescues", "ALTA", 49500),
        ("A Lion Cub ABANDONED by Its Pride | The Rescue That Changed Everything", "Big Cat Rescues", "ALTA", 40500),
        ("The Most DANGEROUS Animal Rescue Ever Attempted | Tiger in a Well", "Big Cat Rescues", "ALTA", 33100),
        ("A Cheetah Cub Found ALONE in the Desert | No One Expected This Ending", "Big Cat Rescues", "ALTA", 22200),
        ("RESCUE: A Baby Elephant TRAPPED in Mud for 3 Days", "Safari & Savanna Rescues", "ALTA", 49500),
        ("A Giraffe TRAPPED on Train Tracks | Will Anyone Get There in Time?", "Safari & Savanna Rescues", "ALTA", 27100),
        ("Baby Antelope Runs for HELP as Its Father Slowly Dies", "Safari & Savanna Rescues", "ALTA", 18100),
        ("A Rhino Mother FIGHTS Poachers to Save Her Baby | Caught on Camera", "Safari & Savanna Rescues", "ALTA", 33100),
        ("The Most EMOTIONAL Elephant Rescue You Will Ever See", "Safari & Savanna Rescues", "MEDIA", 14800),
        ("A Baby Whale STRANDED on the Beach | 200 People Try to Save It", "Ocean & Marine Rescues", "ALTA", 40500),
        ("Dolphin TANGLED in Fishing Net Asks Diver for HELP", "Ocean & Marine Rescues", "ALTA", 27100),
        ("Sea Turtle with 100 BARNACLES | The Painful Rescue Process", "Ocean & Marine Rescues", "ALTA", 22200),
        ("A Great White Shark TRAPPED in a Cave | Divers Risk Everything", "Ocean & Marine Rescues", "MEDIA", 18100),
        ("Baby Seal SEPARATED from Mother During Storm | Will They Reunite?", "Ocean & Marine Rescues", "MEDIA", 12100),
        ("A Puppy Found in a SEALED Box by the River | Who Did This?", "Baby Animal Rescues", "ALTA", 74000),
        ("Baby Fox CRYING for Help in the Snow | No One Expected What Happened", "Baby Animal Rescues", "ALTA", 33100),
        ("An Orphaned Bear Cub Follows a Hiker | The Decision That Changed Both Lives", "Baby Animal Rescues", "ALTA", 22200),
        ("3 Kittens ABANDONED in a Storm Drain | The Race Against Time", "Baby Animal Rescues", "MEDIA", 18100),
        ("Baby Deer Found FROZEN in the Ice | The Warmth That Saved Its Life", "Baby Animal Rescues", "MEDIA", 14800),
        ("Animals TRAPPED in a Wildfire | The Heroes Who Ran INTO the Flames", "Extreme Weather Rescues", "ALTA", 40500),
        ("A Flood Separates a Dog from Its Family | 47 Days Later...", "Extreme Weather Rescues", "ALTA", 27100),
        ("Horses TRAPPED in Rising Floodwaters | The Helicopter Rescue", "Extreme Weather Rescues", "MEDIA", 18100),
        ("Animals Surviving a VOLCANIC Eruption | Nature at Its Most Brutal", "Extreme Weather Rescues", "MEDIA", 14800),
        ("The Cat That Survived an EARTHQUAKE | Found After 16 Days", "Extreme Weather Rescues", "MEDIA", 9900),
        ("A Mother Orangutan REFUSES to Let Go of Her Baby | Loggers Want Them Gone", "Baby Animal Rescues", "ALTA", 33100),
        ("The Dog That Waited 3 YEARS for Its Owner | The Reunion Will Break You", "Baby Animal Rescues", "ALTA", 49500),
        ("A Blind Eagle Rescued from CERTAIN Death | Now It Can Fly Again", "Safari & Savanna Rescues", "MEDIA", 12100),
        ("Wolf Pack SURROUNDS a Lost Child | What Happens Next Defies Nature", "Extreme Weather Rescues", "MEDIA", 22200),
        ("The LARGEST Animal Rescue Operation in History | 10,000 Animals Saved", "Extreme Weather Rescues", "MEDIA", 8100),
    ]
    for i, (title, pillar, pri, vol) in enumerate(titles):
        save_idea(pid, i+1, title, "", "", pillar, pri, search_volume=vol, trending=1)
    log_activity(pid, "project_seeded", f"RESCUE seeded: 5 niches, {len(titles)} titles")
