...
def _run_migrations():
    ...
    migrations = [
        ...
        # v10.1: A/B swap count (limite 1 swap por video — regra Paddy Galloway)
        ("progress_ab_swap_count", "ALTER TABLE progress ADD COLUMN ab_swap_count INTEGER DEFAULT 0"),
        ("progress_ab_swapped_at", "ALTER TABLE progress ADD COLUMN ab_swapped_at TEXT DEFAULT ''"),
        # v10.2: Checklist de producao por card (JSON: {roteiro:bool, narracao:bool, cenas_agente, cenas_flow, edicao, thumb, seo})
        ("progress_checklist", "ALTER TABLE progress ADD COLUMN production_checklist TEXT DEFAULT '{}'"),
        # v8.9: Notifications
    ...
    with get_db() as conn:
        rows = conn.execute(
            """SELECT p.*, p.completed_at, p.ab_decision, p.ab_decided_at, p.video_url,
                      p.ab_swap_count, p.ab_swapped_at, p.production_checklist, p.notes,
